        ...
        try {
            if (Minecraft.getMinecraft().func_147104_D().serverIP.toLowerCase(Locale.ENGLISH).contains("hypixel.net")) {
                if (args.length >= 2) {
                    ...
                    String reportURL = "https://hypixel.net/forums/report-abuse-hackers.37/create-thread";
                    ...
                    new FormattedMessage("Report has been copied to the clipboard. ", EnumChatFormatting.DARK_AQUA).send();
                    new FormattedMessage("Click here to report " + name + ".", EnumChatFormatting.GRAY).addFormatting(EnumChatFormatting.UNDERLINE)
                    .makeClickable(ClickEvent.Action.OPEN_URL, reportURL, new FormattedMessage("Click here to open the thread creation page.", EnumChatFormatting.GRAY)).send();
                } else {
